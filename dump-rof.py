...
def main() -> None:
    ...
    if len(sys.argv) != 3:
        print("Usage: dump-rof.py <path to ROF file> <path to output>")
        return
    ...
    with open(sys.argv[1], "rb") as rof_file:
        parse_directory(rof_file, 0, sys.argv[2])
...
def parse_directory(file: BufferedReader, offset: int, cwd: str, indent: int = 0):
    """
    Reads metadata for the directory at the given offset. Recurses for every directory found
    and calls parse_file() for every file found. Creates the directory under the path given.
    """
    ...
    for dir_name, dir_offset, _ in directories:
        ...
        os.mkdir(cwd + os.sep + dir_name)
        parse_directory(file, dir_offset, cwd + os.sep + dir_name, indent + 1)

    for file_name, file_offset, file_size in files:
        # Print file name
        print(f"{'  ' * (indent + 1)}{file_name}")
        parse_file(file, file_offset, file_size, file_name, cwd)


def parse_file(file: BufferedReader, offset: int, size: int, name: str, cwd: str):
    """
    Extracts the file with given offset and length to the given path and file name.
    """
    ...
    with open(cwd + os.sep + name, "wb") as output_file:
